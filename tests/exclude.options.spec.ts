...
describe('create include option plugin', () => {
  ...
  test('string type exclude options should inject multi correct env', () => {
    const compiler = webpack({
      entry: './entry.js',
      mode: 'production',
      plugins: [new DefineEnvWebpackPlugin({
        exclude: 'GITHUB_ACTIONS'
      })]
    });
  ...
  test('array type exclude options should inject multi correct env', () => {
    const compiler = webpack({
      entry: './entry.js',
      mode: 'production',
      plugins: [new DefineEnvWebpackPlugin({
        exclude: ['GITHUB_ACTIONS', 'GITHUB_REPOSITORY']
      })]
    });
  ...
  test('function type exclude options should inject multi correct env', () => {
    const compiler = webpack({
      entry: './entry.js',
      mode: 'production',
      plugins: [new DefineEnvWebpackPlugin({
  ...
  test('regexp type exclude options should inject multi correct env', () => {
    const compiler = webpack({
      entry: './entry.js',
      mode: 'production',
      plugins: [new DefineEnvWebpackPlugin({
